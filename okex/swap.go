...
func (self *Swap) InitKeys(keys ...string) {
	self.Apikey = keys[0]
	self.SecretKey = keys[1]
	self.PhraseKey = keys[2]
}

func (self *Swap) Period(period base.PERIOD) string {
	var v string
	switch period {
	case base.MIN_1:
		v = "60"
...
func (self *Swap) Depth(depth base.DEPTH) string {
	var v string
	switch depth {
	case base.DEPTH_0:
		v = "0.1"
...
// 订单类型
func (self *Swap) OrderType(order base.ORDER_TYPE) string {
	var v string
	switch order {
...
func (self Swap) header(request string, path string, body interface{}) map[string]string {
...
// 合约持仓信息
func (self Swap) Position() FuturePosition {
	var api = "/api/swap/v3/position"
...
// 单个合约持仓信息
func (self Swap) InstrumenPosition(instrumenid string) (FutureInstrumentPosition, error) {
	var api = fmt.Sprintf("/api/swap/v3/%s/position", instrumenid)
	var url = self.Url + api
...
// 所有币种合约账户信息
func (self Swap) Account() (FutureAccount, error) {
	var api = "/api/swap/v3/accounts"
	var url = self.Url + api
...
// 单个币种合约账户信息
func (self Swap) SymbolAccount(symbol string) (FutureSymbolAccount, error) {
	var api = fmt.Sprintf("/api/swap/v3/accounts/%s", symbol)
	var url = self.Url + api
...
// 获取合约币种杠杆倍数
func (self Swap) Times(symbol string) FutureTimes {
	var api = fmt.Sprintf("/api/swap/v3/accounts/%s/leverage", symbol)
	var url = self.Url + api
 ...
 * time 倍数(10|20)
 */
func (self Swap) SetTimes(symbol string, id string, direct string, time int32) FutureTimes {
	var api = fmt.Sprintf("/api/swap/v3/accounts/%s/leverage", symbol)
	var url = self.Url + api
...
// 账单流水查询
func (self Swap) Ledger(symbol string) (FutureLedger, error) {
	var api = fmt.Sprintf("/api/swap/v3/accounts/%s/ledger", symbol)
	var url = self.Url + api
 ...
 */
func (self Swap) Order(instrumentid string, _type int32, ordertype int32, price float32, size int32, match_price int32) (SwapOrder, error) {
	var api = "/api/swap/v3/order"
	var url = self.Url + api
...
// 撤销指定订单
func (self Swap) CancelOrder(symbol string, orderid string) (FutureCancel, error) {
	var api = fmt.Sprintf("/api/swap/v3/cancel_order/%s/%s", symbol, orderid)
	var url = self.Url + api
...
func (self Swap) List(symbol string, status int) FutureList {
	var api = fmt.Sprintf("/api/swap/v3/orders/%s", symbol)
	var url = self.Url + api
...
// 获取订单信息
func (self Swap) OrderInfo(symbol string, orderid string) FutureOrderInfo {
	var api = fmt.Sprintf("/api/swap/v3/orders/%s/%s", symbol, orderid)
	var url = self.Url + api
...
// 获取成交明细
func (self Swap) Fills(symbol string, orderid string) FutureFill {
	var api = fmt.Sprintf("/api/swap/v3/fills?instrument_id=%s&order_id=%s", symbol, orderid)
	var url = self.Url + api
...
// 获取合约信息
func (self Swap) Instruments() []FutureInstrument {
	var api = "/api/swap/v3/instruments"
	var url = self.Url + api
...
// 获取深度数据
func (self Swap) Depths(symbol string) FutureDepth {
	var api = fmt.Sprintf("/api/swap/v3/instruments/%s/depth", symbol)
	var url = self.Url + api
...
// 获取全部ticker信息
func (self Swap) TickerAll() FutureTickers {
	var api = "/api/swap/v3/instruments/ticker"
	var url = self.Url + api
...
// 获取某个ticker信息
func (self Swap) Ticker(instrument string) float32 {
	var api = fmt.Sprintf("/api/swap/v3/instruments/%s/ticker", instrument)
	var url = self.Url + api
...
// 获取成交数据
func (self Swap) Trades(symbol string, limit int32) ([]Trades, error) {
	var api = fmt.Sprintf("/api/swap/v3/instruments/%s/trades?limit=%d", symbol, limit)
	var url = self.Url + api
...
func (self Swap) Candle(instrumentid string, interval int32, st time.Time) (FutureCandles, error) {
	var api string
	...
	api = fmt.Sprintf("/api/swap/v3/instruments/%s/candles?start=%s&granularity=%d", instrumentid, start, interval)
...
// 获取指数信息
func (okex Swap) Index(symbol string) FutureIndex {
	var api = fmt.Sprintf("/api/swap/v3/instruments/%s/index", symbol)
...
// 获取法币汇率
func (self Swap) Rate() FutureRate {
	var api = "/api/swap/v3/rate"
	var url = self.Url + api
...
// 获取平台总持仓量
func (self Swap) OpenInterest(symbol string) FutureOpenInterest {
	var api = fmt.Sprintf("/api/swap/v3/instruments/%s/open_interest", symbol)
	var url = self.Url + api
	// log.Print(url)
...
// 获取合约当前交易的最高买价和最低卖价
func (self *Swap) Price(instrumentid string) float32 {
	var api = fmt.Sprintf("/api/swap/v3/instruments/%s/price_limit", instrumentid)
	var url = self.Url + api
...
// 获取当前限价
func (self Swap) MarkPrice(symbol string) FutureMarkPrice {
	var api = fmt.Sprintf("/api/swap/v3/instruments/%s/mark_price", symbol)
	var url = self.Url + api
...
// 获取爆仓单
func (self Swap) Liquidation(symbol string, status int) FutureLiquidation {
	var api = fmt.Sprintf("/api/swap/v3/instruments/%s/liquidation?status=%d", symbol, status)
	var url = self.Url + api
	// log.Print(url)
...
// 获取合约挂单冻结数量
func (self Swap) Holds(symbol string) FutureHold {
